...
name = "ExtendingOmniglot"


class ExtendingOmniglot(BaseExperiment):
    ...
    def setup(self):
        ...
        # initialize model
        self.model = Encoding(
            character_size=self.config["input_size"],
            participant_size=self.config["participant_size"],
            ...
            inf=self.config["mode"] == "one_shot_inf",
            infer_target=self.config["infer_target"])
        ...
        self.logger = Logger(
            path=self.log_directory, log_interval=self.config["log_interval"], train_len=len(
                ...
                self.train_loader), num_test_batches=len(self.test_loader),
            num_oneshot_batches=len(self.oneshot_loader),
                ...
                for i in range(self.global_step, self.global_step + batch_size):
                    if i % self.config["plot_every"] == 0:
                        ...
                        self.plotter.scatter_multiple(
                            ...
                            path=self.plot_directory + f"/train_{epoch}_{part}_{char}_{i}")
    ...
    def test(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, (targets, inputs_character, inputs_participant, lengths,
                            (label_parts, label_chars, label_insts)) in enumerate(
                self.test_loader):
                tries = 0
                while True:
                        ...
                        if i % self.config["test_plot_every"] == 0:
                            ...
                            self.plotter.scatter_multiple(
                                [((x, y, p, s), {"size": 100, "color": plt.get_cmap("viridis"), "marker": "o"}),
                                 ((tx, ty, tp, ts),
                                  {"size": 100, "color": plt.get_cmap("plasma"), "marker": "D"})],
                                path=self.plot_directory + f"/test_{epoch}_{part}_{char}_{i}")
