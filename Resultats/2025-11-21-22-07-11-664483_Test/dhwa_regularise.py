...
SUBJECTS_ = ["033"] # Pour selectionner que le sujet 002 par exemple
...
ALL    = list(range(0 , 62))
DATASET = LOWER + UPPER
# On sélectionne seulement les indices de 10 à 20 (les 10 premières lettres minuscules 'a' à 'j')
#DATASET = list(range(10, 20))
...
class CustomDataset(torch.utils.data.IterableDataset) :
    ...
    """
        ...
        The trajectories are stored in a numpy-array of size (62, 5, 250, 8).
        This corresponds to 
            62 different symbols 
            with 5 instances each, 
            a maximum trajectory length of 250, 
            as well as: 
                x-coordinate, 
                y-coordinate,
                pressure, 
                pen_down, 
                timestamp, 
                if the point was marked as "cleaned" (to be ignored), 
                if the point was extrapolated, 
                and if the point was sampled (see below)
    """
