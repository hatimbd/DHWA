...
if torch.cuda.is_available():
    ...
    print(f"\n🚀 ACCÉLÉRATION ACTIVÉE : Utilisation de {torch.cuda.get_device_name(0)}")
    print(f"   Mémoire disponible : {torch.cuda.get_device_properties(0).total_memory / 1e9:.2f} GB\n")
else:
    device = torch.device("cpu")
    print("\n⚠️ ATTENTION : GPU non détecté. Le code tournera lentement sur le CPU.\n")
        ...
        for fold in range(0, len(TRAIN_INSTANCES) + 1) :
                ...
                if fold != len(TRAIN_INSTANCES) :
                    ...
                    with torch.no_grad():
                        for val_data, val_targets in iter(validation_dataloader):
                            # Envoi des données de validation sur le GPU
            ...
            with torch.no_grad():
                for data_, target_ in iter(test_dataloader):
                    # Envoi des données de test sur le GPU
