...
TAG = "Test_Regularization" # J'ai changé le Tag pour que les résultats aillent dans un nouveau dossier
...
# SUBJECTS = 0 # 0 for all subjects
SUBJECTS = 1 # Number of subjects to select randomly
# SUBJECTS_ = [] # Subject selection with leading 0 of 3 digits or []
SUBJECTS_ = ["002"] # Pour selectionner que le sujet 002 par exemple (ou 007)
...
TRAIN_INSTANCES = list(range(1, 5)) # TRAIN_INSTANCES est [1, 2, 3, 4]
TEST_INSTANCES  = [0]
...
class Logger:
    ...
    # Initialise le logger. 'filename' est le chemin du fichier Log.txt.
...
# Elle gère la lecture des fichiers, la transformation des données et la création des lots (batches).
class CustomDataset(torch.utils.data.IterableDataset) :

    """
        https://github.com/CognitiveModeling/DigiLeTs :
        (description des données)
    """
    
    # Prépare le dataset.
    # 'subject' est l'ID du sujet à authentifier (ex: "002").
    # Les données sont lues et formatées. La cible (output) est '1' si les données appartiennent au 'subject', '0' sinon.
    def __init__(self, directory, subject, characters, instances) :
        ...
        if subject is not None :
            # Crée un déséquilibre : garde 1 copie de chaque "autre" sujet, et N copies du "bon" sujet.
            self.files = numpy.repeat(self.files, [number if file_ == subject else 1 for file_ in self.files], axis=0)
        random.shuffle(self.files)
        self.inputs  = numpy.zeros((len(self.files), len(self.characters), 5, 2, 250))
        ...
        for file_, file in enumerate(self.files) :
            ...
            # Ne garde que les coordonnées x, y (:2) et réorganise les axes
            self.inputs[file_] = numpy.swapaxes(numpy.nan_to_num(content["trajectories"][self.characters, :, :, :2]), 2, 3) # (<=62, 5, 250, 2), (<=62, 5, 2, 250)
            if subject is None :
                self.outputs[file_] = int(content["wid"])
            else :
                # Cible (output) = 1 si le sujet est le bon, 0 sinon.
                self.outputs[file_] = int(content["wid"] == subject.split("-")[0])

    # Retourne la taille totale du dataset (nombre de fichiers * nombre d'instances par fichier).
    def __len__(self) :
        return len(self.files) * len(self.instances)

    # Crée un itérateur pour fournir les données au DataLoader.
    # 'yield' renvoie un échantillon et sa cible (output) à la fois.
    def __iter__(self) :
        
        for file_, _ in enumerate(self.files) :
            for instance in self.instances :
                yield self.inputs[file_][:, instance:instance+1, :, :], self.outputs[file_] # (<=62, <=3, 250, 2)

    # Calcule et retourne la taille (nombre de neurones) requise pour la couche d'entrée et la couche de sortie du réseau.
    def size(self) :
        inputs = 0
        outputs = 0
        for file_, _ in enumerate(self.files) :
            # Taille d'entrée = (nombre de caractères * 2 (x,y))
        ...
        # +1 car la sortie est binaire (0 ou 1), donc max(output) sera 1, mais il faut 2 neurones.
...
class Net(torch.nn.Module):
    ...
    # Construit les couches du réseau : une série de couches Linéaires (torch.nn.Linear)
    # suivies de neurones à fuite (snntorch.Leaky).
    def __init__(self, inputs, outputs):

        super().__init__()
        
        # Définit les couches : [couche_entree, couche_cachee_1, ..., couche_sortie]
        self.layers = [inputs] + HIDDEN + [outputs]
        
        self.linear = torch.nn.ModuleList()
        self.leaky = torch.nn.ModuleList()
        
        # Crée une couche linéaire et une couche "leaky" pour chaque transition de couche
        for layer, _ in enumerate(self.layers[:-1]) :
            self.linear.append(torch.nn.Linear(self.layers[layer], self.layers[layer+1]))
            self.leaky.append(snntorch.Leaky(beta=BETA, learn_threshold=True)) # threshold=THRESHOLD, learn_threshold=True
    ...
    # Les données sont traitées pas à pas dans le temps (step by step)
    # pour simuler le comportement temporel du SNN.
    def forward(self, inputs):

        # Initialise le potentiel de membrane de tous les neurones à zéro
        membrane_value = list()
        ...
        # Boucle sur chaque pas de temps (les 250 points de la trajectoire)
        for step in range(inputs.shape[-1]) :
            
            # Prend les données pour ce pas de temps
            numerical_value = inputs[:, :, step]
            # Fait passer les données à travers chaque couche
            for layer, _ in enumerate(self.layers[:-1]) :
                numerical_value = self.linear[layer](numerical_value)
                # Calcule le spike et le nouveau potentiel de membrane
                spike_value, membrane_value_ = self.leaky[layer](numerical_value, membrane_value[layer])
                membrane_value[layer] = membrane_value_
            
            # Enregistre les spikes et le potentiel de membrane de la *dernière* couche
            spike_record.append(spike_value)
            membrane_record.append(membrane_value_)

        # Retourne l'historique de la membrane et des spikes pour tous les pas de temps
        return torch.stack(membrane_record, dim=0), torch.stack(spike_record, dim=0)


# Classe utilitaire pour arrêter l'entraînement si la performance (validation loss) ne s'améliore plus.
class EarlyStopping:
    
    # Initialise l'état de la patience, le delta minimum d'amélioration, et le meilleur score.
    ...
    # Logique principale. Appelée à chaque époque, elle compare la 'val_loss' actuelle
    # avec la meilleure précédente et met à jour le compteur de patience.
    ...
    # Restaure les poids du modèle qui ont donné la meilleure performance (le 'best_model_state').
    def load_best_model(self, model):
        model.load_state_dict(self.best_model_state)


# Fonction utilitaire pour formater et afficher la barre de progression,
# la perte (loss) et la précision (accuracy) dans la console.
...
# Crée un dossier unique pour cette exécution, basé sur la date, l'heure et le TAG
output = os.path.join(OUTPUT, "_".join([datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S-%f"), TAG]))
os.makedirs(output) # Utilisation de makedirs pour créer 'Resultats' si nécessaire


# Copie le script actuel dans le dossier de résultats pour la traçabilité
shutil.copy2(__file__, os.path.join(output, os.path.basename(__file__)))


# Redirige tous les 'print' vers le Logger (console + fichier Log.txt)
sys.stdout = Logger(os.path.join(output, "Log.txt"))


# Sélectionne les sujets à traiter basé sur les variables globales SUBJECTS et SUBJECTS_
...
if SUBJECTS == 0 :
    ...
    if len(SUBJECTS_) == 0 :
        ...
        # Sélectionne N sujets au hasard
        subjects = numpy.random.default_rng().choice(files, size=SUBJECTS, replace=False)
    else :
        # Sélectionne les sujets spécifiés dans la liste SUBJECTS_
        subjects = list()
        for file in files :
...
# Définit le périphérique (GPU si disponible, sinon CPU)
device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")


# Initialise les listes pour stocker les modèles et les précisions pour chaque sujet
...
# Boucle principale sur chaque sujet sélectionné
for subject_, subject in enumerate(subjects) :
    ...
    # Prépare le dataset de test (instance [0])
    ...
    # Boucle de validation croisée (K-Fold)
    # s'exécute pour fold = 0, 1, 2, 3 (validation) et 4 (final)
    for fold in range(0, len(TRAIN_INSTANCES) + 1) :
        ...
        if fold != len(TRAIN_INSTANCES) :
            ...
            # Le dernier fold utilise toutes les instances de train, sans validation
            print(f"No fold\n", flush=True)

            
        train_instances = list(TRAIN_INSTANCES)
        if fold != len(TRAIN_INSTANCES) :
            # Sépare les instances de train et de validation pour ce fold
        ...
        # Crée les datasets et dataloaders pour ce fold
        ...
        # Crée une nouvelle instance du réseau (Net) pour ce fold
        nets[subject_].append(Net(*train_dataset.size()).to(device))

        
        # Initialise la fonction de perte, l'optimiseur et l'early stopping pour ce fold
        loss_calculation = torch.nn.CrossEntropyLoss()
        
        # --- MODIFICATION ICI ---
        # Ajout de 'weight_decay=1e-5' pour la régularisation L2
        optimizer = torch.optim.Adam(nets[subject_][-1].parameters(), lr=LR, betas=BETAS, weight_decay=1e-5)
        ...
        # Boucle d'entraînement pour les EPOCHS (ex: 1000)
        for epoch in range(1, EPOCHS + 1):
            ...
            for train_data, train_targets in iter(train_dataloader) :
                ...
                # Passe les données dans le réseau
                # train_data.view(...) aplatit les données d'entrée
                ...
                # Calcule la perte (loss) pour chaque pas de temps
                ...
                # Calcule la précision (accuracy) en sommant les sorties sur tous les pas de temps
                _,  train_index =  train_results.sum(dim=0).max(1) # Précision "Membrane"
                _, _train_index = _train_results.sum(dim=0).max(1) # Précision "Spike"
            ...
            if fold != len(TRAIN_INSTANCES) :
                ...
                # Désactive le calcul du gradient pour la validation
                with torch.no_grad():
                ...
                # Vérifie l'Early Stopping basé sur la perte de validation
                ...
                # S'il n'y a pas de validation (Fold #4), vérifie l'Early Stopping sur la perte d'entraînement
            ...
            if  abort == True :
                ...
                # Charge le meilleur modèle sauvegardé par l'Early Stopping
                early_stopping.load_best_model(nets[subject_][-1])
                break   
        
                
        # --- PHASE DE TEST ---
        # Cette section est exécutée à la fin de chaque FOLD
        ...
        # Crée et sauvegarde les graphiques de perte (loss)
        ...
        # Crée et sauvegarde les graphiques de précision (accuracy) "Membrane"
        matplotlib.pyplot.figure(facecolor="w", figsize=(10, 5))
        ...
        # Crée et sauvegarde les graphiques de précision (accuracy) "Spikes"
        matplotlib.pyplot.figure(facecolor="w", figsize=(10, 5))
        ...
        # Évalue le modèle final (le meilleur de ce fold) sur le set de test (Instance 0)
        with torch.no_grad():
            ...
            # Crée et sauvegarde les matrices de confusion
    ...
    # Affiche le résumé pour le sujet qui vient de se terminer
    if SUBJECTS == 0 :
    ...
    # Affiche la moyenne et l'écart-type pour ce sujet
...
# --- FIN DE TOUTES LES BOUCLES ---

# Affiche le résumé final pour tous les sujets (si plus d'un sujet a été traité)
if SUBJECTS != 0 :
    print("Summary", flush=True)
    for subject_, subject in enumerate(subjects) :
    ...
    # Affiche la moyenne et l'écart-type de TOUS les sujets combinés
    print("TOTAL", flush=True)
...
# Écrit le résumé final dans un fichier CSV
