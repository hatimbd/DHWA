...
# Tag
TAG = "Test_Optimized_SelectedChars"
...
# --- MODIFICATION 1 : SÉLECTION DES CARACTÈRES SPÉCIFIQUES ---
...
# --- MODIFICATION 2 : OPTIMISATION ---
DOWNSAMPLE = 2 # Prend 1 point sur 2 (Réduit le temps de 250 à 125 étapes)

# Network
HIDDEN = [256] # Réduit de 1000 à 256 pour accélérer le calcul (suffisant pour 4 classes)
BETA = 0.95

# Training
LR = 1e-3 # Légère augmentation du LR car le batch est plus grand
BETAS = (0.9, 0.999)
BATCH = 32 # Augmenté de 8 à 32 pour paralléliser davantage
EPOCHS = 1000
PATIENCE = 10 # Augmenté un peu car l'entraînement est plus rapide
DELTA = 0.01  # Ajusté pour être plus fin
...
class CustomDataset(torch.utils.data.IterableDataset) :
    ...
    def __init__(self, directory, subject, characters, instances) :
        ...
        # Calcul de la taille temporelle après downsampling
        self.time_steps = 250 // DOWNSAMPLE
        
        # Initialisation du tableau avec la nouvelle dimension temporelle
        self.inputs  = numpy.zeros((len(self.files), len(self.characters), 5, 2, self.time_steps))
        ...
        for file_, file in enumerate(self.files) :
            ...
            # Extraction des trajectoires spécifiques
            # Dimensions originales : (Nb_Chars, 5, 250, 2)
            # On applique le downsampling sur l'axe temporel (index 2)
            raw_trajectories = content["trajectories"][self.characters, :, ::DOWNSAMPLE, :2] 
            
            # Swap axes pour avoir (Nb_Chars, 5, 2, TimeSteps)
...
class Net(torch.nn.Module):
    ...
    def __init__(self, inputs, outputs):
        ...
        for layer, _ in enumerate(self.layers[:-1]) :
            ...
            # Threshold adjusté car moins de neurones/entrées peuvent changer la dynamique
    ...
    def forward(self, inputs):
        ...
        # Boucle sur les étapes temporelles (réduite par DOWNSAMPLE)
...
# Classe utilitaire pour Early Stopping
class EarlyStopping:
...
if SUBJECTS == 0 :
    ...
    if len(SUBJECTS_) == 0 :
        print(f"Subjects found : {len(files)}", flush=True)
    ...
    for fold in range(0, len(TRAIN_INSTANCES) + 1) :
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            for train_data, train_targets in iter(train_dataloader) :
                ...
                # Aplatissement des données : (BATCH, features, time)
        ...
        with torch.no_grad():
            ...
            if size > 0:
                ...
                test_accuracies[subject_].append(0)
                _test_accuracies[subject_].append(0)
            ...
            # Confusion Matrix Generation (only if we have data)
            if size > 0:
...
open(os.path.join(output, "Summary.csv"), "w").write(
    ...
    + f"\nAll;Mean+/-Std;"
    + f"{numpy.mean( test_accuracies):06.2f}%+/-{numpy.std( test_accuracies):06.2f};"
    + f"{numpy.mean(_test_accuracies):06.2f}%+/-{numpy.std(_test_accuracies):06.2f}"
    )